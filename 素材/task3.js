...
const server =  http.createServer(function(req,res){
    ...
    let data = JSON.stringify(chapterList[params.chapterId-1]);
    // console.log(params.chapterId);
       ...
       if(pathName=='/list/'){
           ...
           pathName='/addChapter.html';
       }
       else if(pathName=='/detail'){
           pathName='/chapter.html';
       }
       else {
      ...
      pathName=pathName.replace('listmanager/','');
      pathName=pathName.replace('addChapter/','');
      pathName=pathName.replace('detail','');
      pathName = decodeURI(pathName);


    //   if(params.chapterId==1||2||3||4){
    //   detail(data,res);
    //   }
    file(pathName,res);
    resigter(req,res);
...
//读取文件
function file(pathName,res){
    var realFilePath = path.join(__dirname,pathName);
    ...
    fs.readFile(pathName.substr(1),function(err, data) {  
        if(err) {  
            ...
            var content =  fs.readFileSync(realFilePath,"binary");    
...
//get响应,1/2/3/4
function detail(data,res){
        res.write(data);
        // res.end();
}

//后台登录
   function resigter(req,res){
    var data = '';
    if(req.method === 'POST' && req.url === '/login/') {
        ...
        req.on('end', () => {
          ...
          if(account.username === 'admin' && account.pwd === 'admin') {
            console.log('user: %s, password: %s', account.username, account.pwd);
            file('/addChapter.html',res);
          } else {
            file('/login.html',res);
          }
        });
